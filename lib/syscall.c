...
#define	SYS_fork	2
#define	SYS_write	4
#define SYS_sleep	5 // user defined
...
int sem_init(sem_t* sem, int value){
	return syscall(INIT,(uint32_t)sem,value,1);
}
int sem_wait(sem_t* sem){
	return syscall(WAIT,(uint32_t)sem,1,1);
}
int sem_post(sem_t* sem){
	return syscall(POST,(uint32_t)sem,1,1);
}
int sem_destroy(sem_t* sem){
	return syscall(DESTROY,(uint32_t)sem,1,1);
}
